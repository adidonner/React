...
class ProductsService {
    ...
    public async getAllProducts(): Promise<ProductModel[]> {
        ...
        if (productsStore.getState().products.length === 0) {
            const response = await axios.get<ProductModel[]>(appConfig.productsUrl); // waiting
            const products = response.data;
            
            // Redux - update golbal state about fetching all products
        ...
        // if there are products in the Redux global state - return them
